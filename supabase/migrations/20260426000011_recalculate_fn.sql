-- 특정 월 마일리지 전체 재계산 함수 (sub_type_ratios 포함)
...
BEGIN
  ...
  -- 현재 멤버의 해당 월 기록만 삭제 (탈퇴자 기록은 보존)
  DELETE FROM club_workout_mileage cwm
  WHERE cwm.year = p_year AND cwm.month = p_month
    AND EXISTS (
      SELECT 1 FROM club_members cm
      WHERE cm.club_id = cwm.club_id AND cm.user_id = cwm.user_id
    );
  ...
  WHERE w.workout_time >= v_start
    AND w.workout_time <  v_end
  ...
  -- 혼합 운동 (sub_type_ratios 있음: 요가, 복싱 등)
  INSERT INTO club_workout_mileage
    (club_id, workout_id, user_id, mileage, year, month, workout_date, calculated_at)
  SELECT
    cm.club_id,
  ...
  FROM workouts w
  JOIN club_members cm ON cm.user_id = w.user_id,
       jsonb_each(w.sub_type_ratios) AS kv
  LEFT JOIN club_mileage_configs cmc
    ON  cmc.club_id  = cm.club_id
    AND cmc.category = w.category
  ...
  WHERE w.workout_time >= v_start
    AND w.workout_time <  v_end
    AND w.sub_type_ratios IS NOT NULL
    AND w.sub_type_ratios != '{}'::jsonb
  GROUP BY cm.club_id, w.id, w.user_id,
           (w.workout_time AT TIME ZONE 'Asia/Seoul')::date
  ON CONFLICT (club_id, workout_id)
    DO UPDATE SET mileage = EXCLUDED.mileage, calculated_at = EXCLUDED.calculated_at;
END;
